...
class Repository:
    ...
    async def get_mailbox_state(self, mailbox: str = "INBOX") -> MailboxStateRecord:
        ...
        return MailboxStateRecord(
            mailbox=mailbox, last_uid=0, last_sync_at=now_str, status="IDLE"
        )

    async def update_mailbox_state(
        self, mailbox: str, last_uid: int, status: str = "IDLE"
    ) -> None:
        now_str = datetime.now(timezone.utc).isoformat()
        await self.db.execute(
            """
    ...
    async def save_message(self, msg: EmailMessage) -> bool:
        ...
        existing = await self.db.fetch_one(
            "SELECT message_id FROM messages WHERE message_id = ?", (msg.id,)
        )
        if existing:
            return False
    ...
    async def save_thread(self, thread: EmailThread) -> None:
        await self.db.execute(
            ...
            (
                ...
                if isinstance(thread.last_message_at, datetime)
                else str(thread.last_message_at),
                thread.status,
            ),
        )

    async def find_thread_by_header(self, message_id_header: str) -> EmailThread | None:
    ...
    async def get_classification(self, message_id: str) -> ClassificationRecord | None:
        row = await self.db.fetch_one(
    ...
    async def get_outbound_messages_by_thread(
        self, thread_id: str
