...
def test_rule_classifier_out_of_office():
    res = RuleClassifier.classify(
        "msg_1",
        "Automatic Reply: Out of office",
        "I will be away from my desk until Monday.",
    )
...
def test_rule_classifier_bounce():
    res = RuleClassifier.classify(
        ...
        "Mail delivery failed: 550 user unknown.",
    )
    assert res is not None
...
def test_rule_classifier_meeting():
    res = RuleClassifier.classify(
        "msg_4",
        "Re: Proposal",
        "Sounds interesting, let's schedule a call this Friday.",
    )
    assert res is not None
...
def test_rule_classifier_pricing():
    res = RuleClassifier.classify(
        "msg_5",
        "Re: Services",
        "How much does it cost? Please share the pricing details.",
    )
    assert res is not None
...
async def test_llm_classifier_rule_delegation():
    ...
    res = await classifier.classify_message(
        "msg_6", "Re: Quick chat", "Let's set up a demo next Tuesday."
    )
    assert res.intent == "REQUEST_MEETING"
