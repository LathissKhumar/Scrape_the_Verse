...
TRANSITIONS = {
    ...
    (
        LeadStage.CONTACTED,
        "email.intent_detected:REQUEST_MORE_INFO",
    ): LeadStage.REQUEST_INFO,
    (
        LeadStage.CONTACTED,
        "email.intent_detected:REQUEST_MEETING",
    ): LeadStage.MEETING_REQUESTED,
    (
        LeadStage.CONTACTED,
        "email.intent_detected:NOT_INTERESTED",
    ): LeadStage.NOT_INTERESTED,
    (
        LeadStage.CONTACTED,
        "email.intent_detected:UNSUBSCRIBE",
    ): LeadStage.NOT_INTERESTED,
    ...
    for key in keys_to_check:
        ...
        if lookup_pair in TRANSITIONS:
            ...
            return (
                new_stage,
                True,
                f"Transition from {current_stage.value} to {new_stage.value} via '{key}'",
            )

    return (
        None,
        False,
        f"No transition defined for stage '{current_stage.value}' on event '{event_type}' (intent: {intent})",
    )
