...
async def benchmark_bounded_concurrency(url_count: int, concurrency_limit: int):
    ...
    results = await scraper._execute_browser_scrape(urls, max_concurrency=concurrency_limit)
    ...
    print(f"  Max Concurrent Coroutines Observed: {max_active} (Configured Limit: {concurrency_limit})")
