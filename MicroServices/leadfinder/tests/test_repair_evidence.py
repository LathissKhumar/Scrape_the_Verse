        ...
        return_value=[
            {
                ...
                "html": "<html><body><div class='item'>Laptop</div></body></html>",
            }
        ]
    )
...
async def test_evidence_collector_detects_transient_recovery():
        ...
        return_value=[
            {
                ...
                "html": "<div class='card'><h2 class='title'>Book</h2></div>",
            }
        ]
    )
    ...
    mock_extractor.extract = AsyncMock(
        return_value=MagicMock(records=[{"title": "Book"}], strategy_used="css")
    )

    mock_validator = MagicMock()
    mock_validator.validate = AsyncMock(
        return_value=ValidationResult(status="healthy", health_score=0.95)
    )
    ...
    task = ScrapingTask(
        task_id="t1", objective="Scrape books", target_urls=["https://example.com"]
    )
    ...
    raw_pages, is_recovered, val_res = await collector.check_transient_recovery(
