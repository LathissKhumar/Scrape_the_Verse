...
async def test_healing_engine_successful_canary_acceptance():
        ...
        return_value=(
            [
                RawPage(
                    ...
                    html="<div class='product'><h2 class='name'>Phone</h2><span class='price'>$500</span></div>",
                )
            ],
            False,
            None,
    ...
    mock_planner.generate_candidates = AsyncMock(
        return_value=[RepairCandidate(plan=plan, score=0.9, rank=1)]
    )

    mock_extractor = MagicMock()
    mock_extractor.extract = AsyncMock(
        return_value=ExtractionResult(
            records=[{"name": "Phone", "price": "$500"}], strategy_used="css"
        )
    )
    ...
    mock_validator.validate = AsyncMock(
        return_value=ValidationResult(
            ...
            record_count=1,
            field_metrics={
                "name": FieldMetric(coverage=1.0, valid_count=1),
    ...
    task = ScrapingTask(
        task_id="t1", objective="Scrape phone", target_urls=["https://example.com"]
    )
    diagnosis = DiagnosisResult(
        root_cause=RootCause.SELECTOR_DRIFT, confidence=0.9, affected_fields=["name"]
    )
    initial_validation = ValidationResult(
        ...
        record_count=1,
        field_metrics={
            "name": FieldMetric(coverage=0.0),
            "price": FieldMetric(coverage=1.0, valid_count=1),
        },
    )
    initial_schema = ExtractionSchema(
        strategy=ExtractionStrategyEnum.CSS,
        fields=[
            FieldRule(name="name", selector=".old-name"),
...
async def test_healing_engine_escalates_when_candidates_exhausted():
    ...
    mock_collector.check_transient_recovery = AsyncMock(
        return_value=(
            [RawPage(url="https://example.com", html="<html>Empty</html>")],
            False,
            None,
        )
    )
    ...
    mock_planner.generate_candidates = AsyncMock(
        return_value=[RepairCandidate(plan=plan, score=0.4, rank=1)]
    )

    mock_extractor = MagicMock()
    mock_extractor.extract = AsyncMock(
        return_value=ExtractionResult(records=[], strategy_used="css")
    )
