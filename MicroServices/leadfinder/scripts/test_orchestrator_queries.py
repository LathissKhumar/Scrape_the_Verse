...
async def evaluate_queries():
    ...
    print(f"Maps Intent Resolution:      Action={res_maps.action} | Status={res_maps.status} | Collector={res_maps.collector_id}")
    ...
    print(f"IndiaMART Intent Resolution: Action={res_im.action} | Status={res_im.status} | Collector={res_im.collector_id}")
    ...
    print(f"Resolution Result:           Action={res_rest.action} | Status={res_rest.status} | Collector={res_rest.collector_id}")
    ...
    req_amazon = ScrapeTargetRequest(
        ...
        fields=[
            FieldDefinition(name="product_name", description="Name of the Amazon product"),
    ...
    with patch.object(service.client, "create_scraper", new_callable=AsyncMock) as mock_cli:
        mock_cli.return_value = "c_mock_amazon_collector"
        res_amazon = await service.resolve_scraper(req_amazon)
        print(f"Resolution Result:           Action={res_amazon.action} | Status={res_amazon.status} | Job_ID={res_amazon.job_id} | Scraper_ID={res_amazon.scraper_id}")
        if res_amazon.action == "create":
            ...
            print(f"    Reason: No existing collector matches amazon.com schema. Initiating new collector creation job.")
