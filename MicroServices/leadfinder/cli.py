...
async def check_brightdata_health(verbose: bool = False):
    ...
    print("\n" + "=" * 65)
    print(f"  {Style.BRIGHT}BRIGHT DATA COLLECTOR & SYSTEM HEALTH CHECK{Style.RESET_ALL}")
    print("=" * 65)
    print(f"  BRIGHTDATA Enabled:           {Fore.GREEN if settings.BRIGHTDATA else Fore.YELLOW}{settings.BRIGHTDATA}{Style.RESET_ALL}")
    print(f"  API Key Configured:           {Fore.GREEN if bool(settings.BRIGHTDATA_API_KEY) else Fore.RED}{'Yes (***' + str(settings.BRIGHTDATA_API_KEY)[-4:] + ')' if settings.BRIGHTDATA_API_KEY else 'No'}{Style.RESET_ALL}")
    print(f"  Discovery Collector ID:       {Fore.CYAN}{settings.BRIGHTDATA_DISCOVERY_COLLECTOR_ID or settings.BRIGHTDATA_COLLECTOR_ID or 'Not Set'}{Style.RESET_ALL}")
    print(f"  Company Profile Collector ID: {Fore.CYAN}{settings.BRIGHTDATA_COMPANY_COLLECTOR_ID or 'Not Set'}{Style.RESET_ALL}")
    print("-" * 65)

    if not client.is_configured:
        print(f"{Fore.RED}[STATUS] Bright Data is NOT fully configured.{Style.RESET_ALL}")
        print("To enable, set BRIGHTDATA=True and provide BRIGHTDATA_API_KEY in .env")
        return

    print(f"{Fore.GREEN}[STATUS] Bright Data credentials configured successfully!{Style.RESET_ALL}")
    print(f"You can run lead generation via: {Style.BRIGHT}python cli.py \"solar panels\" --leads{Style.RESET_ALL}")
    print(f"Or run query fast-path via:      {Style.BRIGHT}python cli.py \"solar panels\" --engine brightdata{Style.RESET_ALL}")
    print("=" * 65 + "\n")
...
async def execute_lead_gen(
    ...
    print(f"{Style.BRIGHT}Profile Enrichment:{Style.RESET_ALL} {'Enabled (Tier 2 PDP)' if enrich else 'Disabled (Tier 1 Only)'}")
    print(f"{Style.BRIGHT}Discovery Collector:{Style.RESET_ALL}{service.pipeline.discovery_collector_id}")
    print(f"{Style.BRIGHT}Company Collector:{Style.RESET_ALL}  {service.pipeline.company_collector_id}")
    print("-" * 65)
    print(f"{Fore.LIGHTCYAN_EX}[PIPELINE]   {Style.RESET_ALL}Executing Chained Lead Extraction...")

    try:
        leads = await service.generate_leads(query=query, enrich_profiles=enrich)

        print("\n" + "=" * 65)
        print(f"  {Style.BRIGHT}LEAD GENERATION RESULTS ({len(leads)} Leads){Style.RESET_ALL}")
        print("=" * 65)
        ...
        if leads:
            for i, lead in enumerate(leads, 1):
                ...
                if lead.get("product_title"):
                    print(f"  {Style.BRIGHT}Product:{Style.RESET_ALL}       {lead.get('product_title')}")
                if lead.get("price"):
                    price_info = lead.get("price")
                    if isinstance(price_info, dict):
                        print(f"  {Style.BRIGHT}Price:{Style.RESET_ALL}         {price_info.get('symbol', '₹')}{price_info.get('value', '')} {price_info.get('currency', '')}")
                    else:
                        print(f"  {Style.BRIGHT}Price:{Style.RESET_ALL}         {price_info}")
                if lead.get("contact_person"):
                    print(f"  {Style.BRIGHT}Contact Person:{Style.RESET_ALL}{Fore.GREEN} {lead.get('contact_person')}{Style.RESET_ALL}")
                if lead.get("gstin"):
                    print(f"  {Style.BRIGHT}GSTIN:{Style.RESET_ALL}          {lead.get('gstin')}")
                if lead.get("established_year"):
                    print(f"  {Style.BRIGHT}Established:{Style.RESET_ALL}    {lead.get('established_year')}")
                if lead.get("nature_of_business"):
                    print(f"  {Style.BRIGHT}Business Type:{Style.RESET_ALL}  {lead.get('nature_of_business')}")
                if lead.get("city") or lead.get("state"):
                    print(f"  {Style.BRIGHT}Location:{Style.RESET_ALL}       {lead.get('city', '')}, {lead.get('state', '')}")
                if lead.get("company_catalog_url"):
                    print(f"  {Style.BRIGHT}Catalog URL:{Style.RESET_ALL}    {lead.get('company_catalog_url')}")
...
async def execute_query(
    ...
    is_brightdata = (
        (engine_choice == "brightdata")
        or (engine_choice == "auto" and settings.BRIGHTDATA and (any("indiamart" in u.lower() for u in urls) or not urls))
    )

    if is_brightdata and engine_choice != "local":
        print("\n" + "=" * 65)
        print(f"  {Style.BRIGHT}SCRAPE THE VERSE - BRIGHT DATA FAST-PATH{Style.RESET_ALL}")
        print("=" * 65)
        print(f"{Style.BRIGHT}Query:{Style.RESET_ALL}        {query}")
        if urls:
            ...
            for u in urls:
                print(f"  - {u}")
        print(f"{Style.BRIGHT}Engine Mode:{Style.RESET_ALL}  {Fore.GREEN}Bright Data Scraper Studio (Cloud Fast-Path){Style.RESET_ALL}")
        print("-" * 65)
        print(f"{Fore.LIGHTCYAN_EX}[PIPELINE]   {Style.RESET_ALL}Executing cloud collectors...")
        ...
        print("\n" + "=" * 65)
        print(f"  {Style.BRIGHT}SCRAPING RESULTS ({len(res.records)} Records){Style.RESET_ALL}")
        print("=" * 65)
        if res.records:
            ...
            if output_path:
                fmt = (output_format or "json").lower()
                content = DataExporter.to_csv(res.records) if fmt == "csv" else DataExporter.to_json(res.records)
                with open(output_path, "w", encoding="utf-8") as f:
                    f.write(content)
                print(f"\n{Fore.GREEN}[EXPORT] Saved {len(res.records)} records to {output_path}{Style.RESET_ALL}")
        else:
            print(f"{Fore.YELLOW}No records returned from Bright Data.{Style.RESET_ALL}")
        return
    ...
    print("\n" + "=" * 65)
    print(f"  {Style.BRIGHT}SCRAPE THE VERSE - NATIVE MULTI-AGENT ENGINE{Style.RESET_ALL}")
    print("=" * 65)
    print(f"{Style.BRIGHT}Query:{Style.RESET_ALL}        {query}")
    if urls:
        print(f"{Style.BRIGHT}Target URLs ({len(urls)}):{Style.RESET_ALL}")
        for u in urls:
            print(f"  - {u}")
    print(f"{Style.BRIGHT}Engine Mode:{Style.RESET_ALL}  {Fore.CYAN}Native Playwright + Ollama Qwen3:8b + Self-Healing{Style.RESET_ALL}")
    print("-" * 65)
    print(f"{Fore.LIGHTCYAN_EX}[PIPELINE]   {Style.RESET_ALL}Starting Multi-Agent Workflow (Planner -> Scraper -> Extractor -> Validator)...")
    ...
    try:
        ...
        if output:
            ...
            if output_path and output.records:
                fmt = (output_format or "json").lower()
                content = DataExporter.to_csv(output.records) if fmt == "csv" else DataExporter.to_json(output.records)
                with open(output_path, "w", encoding="utf-8") as f:
                    f.write(content)
