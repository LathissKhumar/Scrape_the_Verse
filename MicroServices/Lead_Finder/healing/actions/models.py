...
class PageAction(BaseModel):
    ...
    selector: str | None = Field(default=None, description="Target CSS/XPath selector.")
    value: str | None = Field(
        default=None, description="Optional payload value or scroll amount."
    )
    timeout_ms: int = Field(
        default=5000, ge=500, le=30000, description="Safe timeout for this step."
    )
    optional: bool = Field(
        default=True, description="If True, execution continues if element not found."
    )
    description: str | None = Field(
        default=None, description="Human-readable rationale."
    )
...
class ActionPlan(BaseModel):
    ...
    actions: list[PageAction] = Field(
        default_factory=list, description="Ordered list of page actions (max 5)."
    )
    description: str = Field(
        default="Autonomous action repair plan", description="Summary rationale."
    )
    wait_after_ms: int = Field(
        default=1000,
        ge=100,
        le=10000,
        description="Post-action DOM stabilization pause.",
    )
