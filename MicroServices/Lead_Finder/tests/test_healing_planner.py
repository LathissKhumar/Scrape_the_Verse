...
async def test_planner_scores_and_ranks_candidates():
    ...
    task = ScrapingTask(
        task_id="t1", objective="Scrape laptops", target_urls=["https://example.com"]
    )
    ...
    raw_pages = [
        RawPage(
            ...
            html="<div class='item'><h2>Laptop</h2><span class='cost'>$999</span></div>",
        )
    ]
    current_schema = ExtractionSchema(
        ...
        fields=[
            ...
            FieldRule(name="price", selector=".old-cost"),
        ],
    )
...
async def test_planner_seeds_candidate_from_memory():
    ...
    raw_pages = [
        RawPage(
            url="https://example.com", html="<div class='item'><h2>Laptop</h2></div>"
        )
    ]
    current_schema = ExtractionSchema(
        strategy=ExtractionStrategyEnum.CSS,
        fields=[FieldRule(name="title", selector=".bad")],
    )
...
async def test_planner_invokes_llm_for_selector_repair():
    ...
    task = ScrapingTask(
        task_id="t1", objective="Scrape products", target_urls=["https://example.com"]
    )
    diagnosis = DiagnosisResult(
        root_cause=RootCause.SELECTOR_DRIFT, confidence=0.9, affected_fields=["price"]
    )
    validation = ValidationResult(status="broken", health_score=0.3)
    raw_pages = [
        RawPage(
            url="https://example.com",
            html="<div class='product'><span class='cost'>$50</span></div>",
        )
    ]
    current_schema = ExtractionSchema(
        strategy=ExtractionStrategyEnum.CSS,
        fields=[FieldRule(name="price", selector=".old-price")],
    )
    ...
    assert any(
        c.plan.proposed_configuration.get("price") == ".cost" for c in candidates
    )
