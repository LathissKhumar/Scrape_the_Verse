...
async def benchmark_extraction_throughput(url_counts: list[int]):
    ...
    for count in url_counts:
        ...
        print(
            f"  Duration: {total_duration:.2f}s | Throughput: {throughput_rps:.1f} RPS ({pages_per_min:.0f} pages/min)"
        )
        print(
...
async def benchmark_concurrency_limits():
    ...
    urls_1m = [
        f"https://example.com/products/item-{i % 500000}" for i in range(1000000)
    ]  # 50% duplicate rate
    t0 = time.time()
    tracemalloc.start()
    ...
    tracemalloc.stop()
    print(
        f"Deduplicated 1,000,000 URLs in {t_dedup:.3f}s -> {unique_count:,} unique URLs."
    )
    ...
    print(
        f"Evaluated 10,000 unique domains in {t_domains:.3f}s (Rate: {10000 / t_domains:.0f} ops/sec)"
    )
