...
def build_prompt_context(
        ...
        important_pages_summary.append(
            {
                "url": p.url,
                "title": p.title,
                "meta_description": p.meta_description[:100]
        ...
        for name, metrics in comp_analysis["comparison_matrix"].items():
            if name != biz.company_name:
                gaps = [
                    ...
                    if v == "not_verified" or v == "unknown"
                ]
                if gaps:
    ...
    context = {
        ...
        "business_summary": {
            ...
            "location": biz.location,
            "primary_services": [
                s.name for s in biz.service_analysis.services if s.importance == "core"
            ],
            "all_services": [s.name for s in biz.service_analysis.services],
            "services_without_dedicated_pages": [
                s.name for s in services_without_pages
            ],
...
def format_context_for_llm(context: dict[str, Any]) -> str:
    ...
    lines.append(
        f"SEO Score: {ws['overall_score']}/100 | Pages: {ws['pages_analyzed']}"
    )
    lines.append(
        f"Categories: {', '.join(f'{k}:{v}' for k, v in ws['category_scores'].items())}"
    )
    ...
    for page in context["important_pages"][:3]:
        lines.append(
            f"  {page['url']} | Title: {page['title'][:30]} | Meta: {page['meta_description'][:30]} | Issues: {','.join(page['issues'][:2])}"
        )
    lines.append("")
    ...
    for prob in context["business_problems"][:2]:
        lines.append(
            f"  {prob['title']}: {prob['problem'][:50]} (Impact:{prob['impact']})"
        )
    lines.append("")
...
def build_structured_output(
    ...
    meta_issues = [
        ...
        if "meta" in f.type.lower() and "description" in f.type.lower()
    ]
    if meta_issues:
        seo_changes.append(
            f"Fix missing/short meta descriptions on {len(meta_issues)} identified pages"
        )

    title_issues = [f for f in seo_findings if "title" in f.type.lower()]
    if title_issues:
        seo_changes.append(
            f"Optimize title tags exceeding 60 characters on {len(title_issues)} identified pages"
        )
    ...
    alt_issues = (
        [f for f in seo.accessibility_findings]
        if hasattr(seo, "accessibility_findings")
        else []
    )
    if alt_issues:
        seo_changes.append(
            f"Add missing alt text to images on {len(alt_issues)} identified pages"
        )

    schema_issues = [
        f
        for f in seo_findings
        if "schema" in f.type.lower() or "structured" in f.type.lower()
    ]
    if schema_issues:
    ...
    internal_link_issues = [
        f
        for f in seo_findings
        if "internal" in f.type.lower() and "link" in f.type.lower()
    ]
    ...
    if services_without_pages:
        ux_changes.append(
            "Improve navigation to specialized service pages missing dedicated pages"
        )

    anxiety_segments = [
        s
        for s in biz.customer_analysis.primary_segments
        if "anxiety" in s.segment_name.lower() or "fear" in s.segment_name.lower()
    ]
    if anxiety_segments:
        ux_changes.append(
            "Add clear trust signals and reassurance paths for anxiety patients"
        )

    contact_pages = [
        p
        for p in seo.important_pages
        if "contact" in p.url.lower() or "booking" in p.url.lower()
    ]
    if contact_pages:
    ...
    mobile_issues = [
        f
        for f in seo_findings
        if "mobile" in f.type.lower() or "viewport" in f.type.lower()
    ]
    if mobile_issues:
    ...
    if services_without_pages:
        content_changes.append(
            f"Create dedicated service pages for: {', '.join([s.name for s in services_without_pages])}"
        )

    thin_content = (
        [f for f in seo.content_findings if "thin" in f.type.lower()]
        if hasattr(seo, "content_findings")
        else []
    )
    if thin_content:
        content_changes.append(
            f"Expand thin content on {len(thin_content)} identified pages"
        )
    ...
    if contact_pages:
        conversion_changes.append(
            "Improve contact form visibility and usability on identified pages"
        )

    mobile_cta_issues = [
        f
        for f in seo_findings
        if "mobile" in f.type.lower()
        and ("cta" in f.type.lower() or "click" in f.type.lower())
    ]
    if mobile_cta_issues:
        conversion_changes.append(
            "Add click-to-call for mobile users on affected pages"
        )
    ...
    if anxiety_segments:
        design_changes.append(
            "Use calming color palette and reassuring visual hierarchy for anxiety-focused content"
        )
    ...
    if hasattr(seo, "accessibility_findings") and seo.accessibility_findings:
        design_changes.append(
            "Ensure sufficient color contrast per accessibility findings"
        )
    ...
    if perf_score < 80:
        technical_changes.append(
            "Optimize Core Web Vitals (LCP, CLS, INP) per performance findings"
        )

    if alt_issues:
        technical_changes.append(
            "Compress and add alt text to images per identified issues"
        )
    ...
    render_blocking = [
        f
        for f in seo_findings
        if "render" in f.type.lower() and "block" in f.type.lower()
    ]
    if render_blocking:
        technical_changes.append(
            "Minimize render-blocking resources per identified issues"
        )
    ...
    for p in seo.important_pages[:10]:
        ...
        seo_reason = (
            f"Address {len(issues_list)} identified SEO issues"
            if issues_list
            else "Optimize per page analysis"
        )

        # Determine page type for appropriate reasoning
        url_lower = p.url.lower()
        is_service = any(
            x in url_lower for x in ["behandeling", "service", "treatment"]
        )
        ...
        is_blog = any(
            x in url_lower for x in ["2023/", "2024/", "2025/", "2026/", "blog", "news"]
        )
    ...
    return StructuredOutput(
        ...
        business_summary={
            ...
            "primary_customers": [
                s.segment_name for s in biz.customer_analysis.primary_segments
            ],
            "business_score": biz.business_score.get("overall_score", 0),
        },
