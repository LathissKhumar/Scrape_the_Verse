...
from extractors import extract_business_intelligence, extract_website_intelligence
from utils import find_json_files, load_json_file, logger, normalize_string
...
def discover_and_preprocess(company_name: str, output_dir: Path) -> tuple:
    ...
    for f in seo_files:
        if (
            normalize_string(Path(f).stem).startswith(normalized)
            or "atlas_audit" in f.lower()
        ):
            seo_report_path = f
            break
