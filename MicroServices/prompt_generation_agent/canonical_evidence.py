...
class CanonicalEvidence:
    ...
    verified_customer_information: List[CanonicalEvidenceItem] = field(default_factory=list)
    verified_contact_information: List[CanonicalEvidenceItem] = field(default_factory=list)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "verified_business_facts": [e.to_dict() for e in self.verified_business_facts],
            "verified_services": [e.to_dict() for e in self.verified_services],
            "verified_customer_information": [e.to_dict() for e in self.verified_customer_information],
            "verified_contact_information": [e.to_dict() for e in self.verified_contact_information],
            "seo_findings": [e.to_dict() for e in self.seo_findings],
            "page_specific_findings": [e.to_dict() for e in self.page_specific_findings],
            "business_problems": [e.to_dict() for e in self.business_problems],
            "business_opportunities": [e.to_dict() for e in self.business_opportunities],
            ...
            "preservation_requirements": [e.to_dict() for e in self.preservation_requirements],
...
def normalize_evidence(
    ...
    # Company name
    name = bp.get("business_name", {}).get("value") or bp.get("official_name", {}).get("value")
    if name and name != "Not specified":
        idx += 1
        evidence.verified_business_facts.append(CanonicalEvidenceItem(
    ...
    # Location
    location = business_intelligence.get("location") or bp.get("geographic_market", {}).get("value")
    if location and location != "Not specified":
        ...
        evidence.verified_contact_information.append(CanonicalEvidenceItem(
    ...
    for svc in sa.get("services", []):
        ...
        if svc_name:
            ...
            evidence.verified_services.append(CanonicalEvidenceItem(
                id=_make_id("svc", idx),
                type=EvidenceType.VERIFIED_FACT,
                source=EvidenceSource.BUSINESS_ANALYSIS,
                category="service",
            ...
            if is_core and not has_page:
                idx += 1
                evidence.recommended_pages.append(CanonicalEvidenceItem(
    ...
    for seg in ca.get("segments", []):
        idx += 1
        evidence.verified_customer_information.append(CanonicalEvidenceItem(
        ...
        for need in seg.get("needs", []):
            idx += 1
            evidence.customer_needs.append(CanonicalEvidenceItem(
    ...
    for cat, score in cat_scores.items():
        idx += 1
        evidence.seo_findings.append(CanonicalEvidenceItem(
    ...
    for page in seo_intelligence.get("important_pages", []):
        ...
        evidence.page_specific_findings.append(CanonicalEvidenceItem(
    ...
    for prob in business_intelligence.get("business_problems", []):
        idx += 1
        evidence.business_problems.append(CanonicalEvidenceItem(
    ...
    for opp in business_intelligence.get("opportunities", []):
        idx += 1
        evidence.business_opportunities.append(CanonicalEvidenceItem(
    ...
    for strength in seo_intelligence.get("strengths", []):
        idx += 1
        evidence.existing_strengths.append(CanonicalEvidenceItem(
    ...
    for item in evidence.verified_business_facts:
        if item.category in ("company_name", "location", "website_url", "industry"):
            idx += 1
            evidence.preservation_requirements.append(CanonicalEvidenceItem(
    ...
    unknown_items = [
        ...
        ("certifications", "Certifications/memberships (KNMT, SBB) not explicitly verified in source"),
    ...
    for key, desc in unknown_items:
        idx += 1
        evidence.unknown_information.append(CanonicalEvidenceItem(
