...
class CTADetector:
    ...
    def analyze_conversion_signals(pages: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if not has_phone_cta:
            conversion_score -= 20
            issues.append(
                "No prominent Click-to-Call CTA button detected above the fold."
            )
        ...
        if not has_live_chat:
            conversion_score -= 15
            issues.append(
                "No instant chat or WhatsApp messaging widget for fast lead capture."
            )
        ...
        return {
            ...
            "conversion_grade": "High"
            if conversion_score >= 80
            else ("Moderate" if conversion_score >= 50 else "Low/Leaking Leads"),
        }
