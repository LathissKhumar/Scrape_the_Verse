...
from LibreCrawl.engine import CrawlJob, format_error_result, validate_url
...
def start_crawl_endpoint():
    ...
    if not url:
        return jsonify(
            format_error_result("MISSING_PARAMETER", "Field 'url' is required.")
        ), 400
    ...
    if not ok:
        ...
        return jsonify(
            format_error_result("CRAWL_START_FAILED", msg, {"url": url})
        ), 500
...
def get_crawl_status(job_id: str):
    ...
    if not job:
        return jsonify(
            format_error_result("JOB_NOT_FOUND", f"No crawl job found with ID {job_id}")
        ), 404
...
def get_crawl_issues(job_id: str):
    ...
    if category:
        issues = [
            i for i in issues if i.get("category", "").lower() == category.lower()
        ]

    return jsonify({"job_id": job_id, "total_issues": len(issues), "issues": issues})
...
def get_crawl_pages(job_id: str):
    ...
    return jsonify({"job_id": job_id, "total_pages": len(pages), "pages": pages})
...
def get_crawl_links(job_id: str):
    ...
    return jsonify({"job_id": job_id, "total_links": len(links), "links": links})
