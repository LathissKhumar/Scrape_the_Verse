...
class Client:
    ...
    def print_history(self, tail=8):
        ...
        print(
            "  last polls (seq sent -> latest, events, reset, epoch, status, crawled):"
        )
        for row in self.history[-tail:]:
            print(
                f"    {row['seq_sent']:>8} -> {row['latest_seq']!s:>8}  "
                f"n={row['events']:<6} reset={row['reset']!s:<5} "
...
def poll_until_done(session, base, client, timeout, pause_test=False):
    ...
    while time.time() < deadline:
        ...
        check(
            len(client.urls) <= crawled,
            f"client holds {len(client.urls)} urls but stats say crawled={crawled}",
        )

        if polls % 5 == 0:
            print(
                f"    poll {polls}: {data['status']} urls={len(client.urls)} "
        ...
        if pause_test and not paused and len(client.urls) >= 5:
            ...
            time.sleep(2)
            before = session.get(
                f"{base}/api/crawl_status", params={"stats_only": 1}, timeout=30
            ).json()["stats"]["crawled"]
            time.sleep(3)
            after = session.get(
                f"{base}/api/crawl_status", params={"stats_only": 1}, timeout=30
            ...
            check(
                after - before <= 2,
                f"crawl kept working while paused ({before} -> {after})",
            )
...
def run(args, client_holder=None):
    ...
    session.get(
        base + "/", timeout=30
    )  # establishes the session (local mode auto-login)
    ...
    configure(
        ...
        crawlExternalLinks=args.external,
        crawlImages=args.images,
        enableJavaScript=args.js,
        jsWaitTime=2,
    ...
    print(
        f"  config: max_urls={args.max_urls} depth={args.depth} delay={args.delay} "
    ...
    started = time.time()
    reply = session.post(
        f"{base}/api/start_crawl", json={"url": args.url}, timeout=60
    ).json()
    start_latency = time.time() - started
    check(reply.get("success"), f"start_crawl failed: {reply}")
    crawl_id = reply.get("crawl_id")
    print(
        f"  start_crawl returned in {start_latency * 1000:.0f}ms (crawl_id={crawl_id})"
    )
    check(
        start_latency < 5,
    ...
    missing_status = [
        key
        for key, link in client.links.items()
    ...
    print(
        f"    urls={len(client.urls)} links={len(client.links)} issues={len(client.issues)}"
    )
    print(f"    status codes: {dict(sorted(codes.items(), key=lambda kv: str(kv[0])))}")
    print(
        f"    url_updates={client.url_updates} link_updates={client.link_updates} "
        f"resets={client.resets}"
    )
    print(
        f"    js_rendered={rendered} with_redirect_chain={redirected} "
        f"offdomain_rows={len(offdomain)}"
    )
    if exports:
        print(
            "    exports: "
            + ", ".join(f"{k}={v[0]} rows/{v[1] // 1024}KB" for k, v in exports.items())
        )
    if replay:
        print(
            f"    saved-crawl replay: urls={replay[0]} links={replay[1]} issues={replay[2]}"
        )
    sizes = final.get("memory_data") or {}
    if sizes:
        print(
            f"    memory: {sizes.get('total_deep_mb', 0):.1f}MB data, "
...
def main():
    ...
    parser.add_argument(
        "--base", default=DEFAULT_BASE, help="LibreCrawl instance to drive"
    )
    parser.add_argument("--max-urls", type=int, default=150)
    parser.add_argument("--depth", type=int, default=4)
    parser.add_argument(
        "--delay", type=float, default=0.25, help="seconds between requests"
    )
    ...
    parser.add_argument(
        "--pause-test", action="store_true", help="also verify pause/resume"
    )
