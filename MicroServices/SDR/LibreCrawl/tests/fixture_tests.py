...
def test_cross_domain_redirect():
    ...
    a = serve(
        make_handler(
            {
                "/": html('<a href="/go">offsite</a><a href="/local.html">local</a>'),
    ...
    b = serve(
        make_handler(
            {
                "/landing": html(
                    '<a href="/issues">issues</a><a href="/pulls">pulls</a>'
                ),
            }
        ),
        site_b,
    ...
    try:
        ...
        phantoms = [
            u for u in urls if f":{site_a}" in u and ("/issues" in u or "/pulls" in u)
        ]
        ...
        result(
            "redirect: chain and destination recorded",
            bool(go.get("redirects"))
            and go.get("redirected_to", "").endswith("/landing"),
        )

        result(
            "redirect: offsite links attributed to the other host",
            f"http://127.0.0.1:{site_b}/issues" in targets,
        )
        result(
            "redirect: offsite content does not enter the queue",
...
def test_external_images():
    ...
    a = serve(
        make_handler(
            {
                "/": html(
                    f'<img src="/in.png"><img src="http://127.0.0.1:{other}/out.png">'
                ),
                "/in.png": png(),
            }
        ),
    ...
    try:
        ...
        urls = {r["url"] for r in crawler.crawl_results}
        result(
            "images: offsite image checked when external crawling is on",
...
def test_sitemap_discovery_is_async():
    ...
    try:
        ...
        result(
            "sitemap: start_crawl returns immediately",
            latency < 0.5,
            f"{latency * 1000:.0f}ms",
        )
        result(
            "sitemap: crawl waits for slow discovery",
...
def test_max_urls_counts_pages_not_images():
    ...
    routes = {
        "/": html(
            "".join(f'<img src="/img{i}.png">' for i in range(30))
    ...
    try:
        ...
        crawler = crawl(
            f"http://127.0.0.1:{site}/", max_urls=limit, concurrency=concurrency
        )
        pages = [
            r
            for r in crawler.crawl_results
...
def test_event_ordering_under_polling():
    ...
    try:
        ...
        result(
            "events: no ordering violations under concurrent polling",
            not problems,
            f"{len(problems)} found: {problems[:3]}",
        )
        result(
            "events: replayed client state matches the crawler",
...
def test_discovered_counts_image_rows():
    ...
    try:
        ...
        result(
            "discovered counts every row, images included",
            discovered >= rows,
            f"discovered={discovered} crawled={crawled} rows={rows}",
        )
        result(
            "discovered is never behind crawled",
            discovered >= crawled,
            f"{discovered} vs {crawled}",
        )
        result("image rows were actually synthesized", rows > 2, f"{rows} rows")
    finally:
        a.shutdown()
...
def test_js_response_time_excludes_render_wait():
    ...
    if not _playwright_available():
        result(
            "js response time excludes the render wait",
            True,
    ...
    try:
        ...
        if not pages:
            result(
                "js response time excludes the render wait", False, "no page crawled"
            )
            return
        ...
        result(
            "response_time excludes the render wait",
            response_ms < wait * 1000,
            f"{response_ms}ms with a {wait}s wait",
        )
        result(
            "response_time stays under the slow threshold for a fast page",
            response_ms < 3000,
            f"{response_ms}ms",
        )
        result(
            "render_time is reported separately and includes the wait",
            render_ms >= wait * 1000,
            f"{render_ms}ms",
        )
        ...
        slow = [
            i
            for i in detector.detected_issues
            if i.get("issue") == "Slow Response Time"
        ]
        result("a fast page is not flagged as slow", not slow, str(slow[:1]))
    finally:
        a.shutdown()
