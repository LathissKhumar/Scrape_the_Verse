...
def init_crawl_tables():
    ...
    with get_db() as conn:
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_crawls_user_status ON crawls(user_id, status)"
        )
        cursor.execute(
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_crawled_urls_status ON crawled_urls(crawl_id, status_code)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_crawl_links_crawl ON crawl_links(crawl_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_crawl_links_source ON crawl_links(crawl_id, source_url)"
        )
        cursor.execute(
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_crawl_issues_category ON crawl_issues(crawl_id, category)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_crawl_queue_crawl ON crawl_queue(crawl_id)"
        )
...
def get_crawl_count(user_id):
    ...
    try:
        with get_db() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT COUNT(*) as count FROM crawls WHERE user_id = ?", (user_id,)
            )
