...
def normalize_issue(
    issue_raw: dict[str, Any], pages_by_url: dict[str, Any] | None = None
) -> dict[str, Any]:
    ...
    if pages_by_url and url in pages_by_url:
        ...
        if "title" in type_key:
            evidence = {
                ...
                "length": len(page.get("title") or ""),
            }
        elif "meta_description" in type_key:
            evidence = {
                "meta_description": page.get("meta_description"),
                "length": len(page.get("meta_description") or ""),
            }
        elif "h1" in type_key:
            ...
            evidence = {
                "response_time_ms": page.get("response_time"),
                "render_time_ms": page.get("render_time"),
            }
...
def normalize_page(page_raw: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "render_time_ms": round(float(page_raw.get("render_time") or 0), 2)
        if page_raw.get("render_time") is not None
        else None,
...
def normalize_crawl_result(
    crawler: WebCrawler, duration: float = 0.0, started_at: str | None = None
) -> dict[str, Any]:
    ...
    if hasattr(crawler, "sitemap_urls") and crawler.sitemap_urls:
        ...
        sitemaps_discovered = list(
            getattr(crawler.sitemap_parser, "discovered_sitemaps", [])
        )
    ...
    return {
        ...
        or (urlparse(crawler.base_url).netloc if crawler.base_url else ""),
        "status": "completed" if not crawler.is_running else "running",
        "summary": {
            "total_pages_crawled": len(normalized_pages),
            "total_pages_discovered": raw_stats.get(
                "discovered", len(normalized_pages)
            ),
    ...
    try:
        ...
        if not ok:
            return format_error_result(
                "CRAWL_START_FAILED", f"Failed to start crawl: {message}", {"url": url}
            )
        ...
        result = normalize_crawl_result(
            crawler, duration=duration, started_at=started_at
        )
        return result
        ...
        return format_error_result(
            ...
            f"An unhandled error occurred during crawl execution: {e!s}",
...
class CrawlJob:
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            else ("completed" if self.crawler.stats.get("crawled", 0) > 0 else "idle"),
    ...
    def get_results(self) -> dict[str, Any]:
        ...
        return normalize_crawl_result(
            self.crawler, duration=duration, started_at=self.started_at
        )
