...
class OpportunityBuilder:
    ...
    @staticmethod
    def build_opportunities_from_audit(audit_state: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for issue in issues[:10]:
            if any(k in str(issue).lower() for k in ["viewport", "responsive", "mobile", "css", "layout", "slow"]):
        ...
        local_evidence = []
        for issue in issues:
            if any(k in str(issue).lower() for k in ["local", "schema", "nap", "address", "map", "json-ld"]):
                local_evidence.append({"finding": issue, "type": "Local SEO"})

        if local_score < 80.0 or schema_score < 70.0 or local_evidence:
            opp_score = round(max(55.0, 100.0 - ((local_score + schema_score) / 2.0)), 1)
            opportunities.append({
                "id": f"opp_{uuid4().hex[:12]}",
                "type": "LOCAL_SEO",
        ...
        if content_score < 80.0 or onpage_score < 80.0:
            opp_score = round(max(50.0, 100.0 - ((content_score + onpage_score) / 2.0)), 1)
            opportunities.append({
                "id": f"opp_{uuid4().hex[:12]}",
                ...
                "evidence": [{"content_score": content_score, "onpage_score": onpage_score}],
                "recommended": opp_score >= 50.0,
                "status": "IDENTIFIED",
            })
        ...
        if not opportunities:
            opportunities.append({
                ...
                "evidence": [{"audit_summary": "Overall baseline passed, advanced CRO recommended."}],
                "recommended": True,
                "status": "IDENTIFIED",
            })
