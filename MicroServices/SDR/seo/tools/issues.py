...
def filter_issues_tool(
    ...
    if issue_type:
        filtered = [i for i in filtered if (i.get('type') or '').lower() == issue_type.lower()]
    if url_substring:
        filtered = [i for i in filtered if url_substring.lower() in (i.get('url') or '').lower()]
    return filtered
...
def get_issue_summary_tool(issues: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "top_issue_types": sorted(by_type.items(), key=lambda x: x[1], reverse=True)[:10]
    }
