...
from seo.state import ActionItem, AuditFinding, SEOState
...
from seo.organizer import organize_website_crawl
...
def main():
    parser = argparse.ArgumentParser(
        description="SEO Agent - LangGraph Powered SEO Audit Engine"
    )
    ...
    parser.add_argument(
        "--max-pages", "-m", type=int, default=100, help="Max pages to crawl"
    )
    parser.add_argument(
        "--javascript",
        "--js",
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable PageSpeed analysis",
    )
    parser.add_argument(
        "--pagespeed-key",
        "-k",
        type=str,
        help="Google PageSpeed API Key (avoids public rate limits)",
    )

    # Export options
    parser.add_argument(
        "--output", "-o", type=str, help="Output file path (.md, .json, or .xlsx)"
    )
    parser.add_argument(
        "--excel", "-x", type=str, help="Export detailed multi-tab Excel file (.xlsx)"
    )
    parser.add_argument(
        "--json-output",
        "-j",
        type=str,
        help="Export complete structured JSON file (.json)",
    )
    parser.add_argument(
        "--export-all",
        type=str,
        help="Export all formats (Markdown, JSON, Excel) with the given filename prefix",
    )
    parser.add_argument(
        "--data-dir",
        default="report",
        help="Base directory for domain-partitioned normalized data (default: report)",
    )
    parser.add_argument(
        "--no-organize", action="store_true", help="Skip domain directory organization"
    )
    parser.add_argument(
    ...
    if final_state.get("status") == "failed":
        print(
            f"\nAudit failed: {', '.join(final_state.get('errors', []))}",
            file=sys.stderr,
        )
        return 1
    ...
    if not args.no_organize and final_state.get("status") == "completed":
        try:
            ...
            print(
                f"  - Issues Deduplicated: {val.get('normalized_issues_count', 0)} (removed {val.get('duplicate_issues_deduplicated', 0)} duplicates)"
            )
            ...
            print(
                f"  - Validation: {'PASSED' if val.get('valid') else 'FAILED'} (Data Loss: {val.get('data_loss')})"
            )
        except Exception as e:
            print(
                f"\n[Data Organizer] Notice: Could not organize domain folders: {e}",
                file=sys.stderr,
            )
