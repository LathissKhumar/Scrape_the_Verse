...
def export_to_excel(state: dict[str, Any], filepath: str) -> bool:
    ...
    if not OPENPYXL_AVAILABLE:
        print(
            "Warning: openpyxl is not installed. Run `pip install openpyxl` to export to Excel."
        )
        return False
    ...
    ws_summary.cell(
        row=1, column=1, value="SEO Audit Executive Overview"
    ).font = title_font
    ws_summary.cell(
        row=2, column=1, value=f"Target URL: {state.get('url', '')}"
    ).font = subtitle_font
    ...
    summary_data = [
        ...
        (
            "Crawl Duration",
            f"{state.get('crawl_summary', {}).get('duration_seconds', 0)} seconds",
        ),
    ...
    ws_summary.cell(row=12, column=1, value="Category Score Breakdown").font = Font(
    ...
    for row_idx, item in enumerate(actions, start=2):
        ...
        ws_actions.cell(
            row=row_idx, column=1, value=f"Priority {prio}"
        ...
        ws_actions.cell(
            row=row_idx, column=5, value=item.get("impact_score", 5)
        ).alignment = Alignment(horizontal="center")
        ws_actions.cell(
            row=row_idx, column=6, value=item.get("estimated_effort", "medium").title()
        ).alignment = Alignment(horizontal="center")
        ws_actions.cell(
            row=row_idx, column=7, value=item.get("affected_count", 0)
        ).alignment = Alignment(horizontal="center")
    ...
    for row_idx, iss in enumerate(issues_list, start=2):
        ...
        ws_issues.cell(
            row=row_idx, column=2, value=iss.get("type", "")
        ).alignment = Alignment(horizontal="center")
        ws_issues.cell(
            row=row_idx, column=3, value=iss.get("category", "")
        ).alignment = Alignment(horizontal="center")
        ...
        evidence_str = (
            json.dumps(iss.get("evidence", {}), default=str)
            if iss.get("evidence")
            else ""
        )
    ...
    for col_idx, h in enumerate(page_headers, start=1):
        ...
        cell.fill = header_fill
        cell.alignment = Alignment(
            horizontal="center" if col_idx in (2, 4, 6, 8, 11, 12) else "left"
        )
    ...
    for row_idx, p in enumerate(pages_list, start=2):
        ...
        ws_pages.cell(row=row_idx, column=4, value=len(title)).alignment = Alignment(
            horizontal="center"
        )
        ws_pages.cell(row=row_idx, column=5, value=meta_desc)
        ws_pages.cell(
            row=row_idx, column=6, value=len(meta_desc)
        ).alignment = Alignment(horizontal="center")
        ws_pages.cell(row=row_idx, column=7, value=p.get("h1", "") or "")
        ws_pages.cell(
            row=row_idx, column=8, value=p.get("word_count", 0)
        ).alignment = Alignment(horizontal="center")
        ws_pages.cell(row=row_idx, column=9, value=p.get("canonical", "") or "")
        ws_pages.cell(row=row_idx, column=10, value=p.get("robots", "") or "")
        ws_pages.cell(
            row=row_idx, column=11, value=p.get("depth", 0)
        ).alignment = Alignment(horizontal="center")
        ws_pages.cell(
            row=row_idx, column=12, value=p.get("response_time_ms", 0)
        ).alignment = Alignment(horizontal="center")
    ...
    for col_idx, h in enumerate(link_headers, start=1):
        ...
        cell.fill = header_fill
        cell.alignment = Alignment(
            horizontal="center" if col_idx in (4, 5, 7) else "left"
        )
    ...
    for row_idx, l in enumerate(links_list, start=2):
        ...
        ws_links.cell(
            row=row_idx, column=4, value="Internal" if is_int else "External"
        ).alignment = Alignment(horizontal="center")

        t_status = l.get("status_code")
        st_cell = ws_links.cell(
            row=row_idx, column=5, value=t_status if t_status is not None else "-"
        )
        ...
        ws_links.cell(
            row=row_idx, column=7, value=l.get("placement", "body")
        ).alignment = Alignment(horizontal="center")
    ...
    for col_idx, h in enumerate(perf_headers, start=1):
        ...
        cell.fill = header_fill
        cell.alignment = Alignment(
            horizontal="center" if col_idx in (2, 3, 4, 5, 6) else "left"
        )

    pagespeed_lookup = {
        ps.get("url"): ps for ps in state.get("pagespeed", []) if ps.get("url")
    }
    for row_idx, p in enumerate(pages_list, start=2):
        p_url = p.get("url", "")
        ...
        ws_perf.cell(
            row=row_idx, column=2, value=p.get("status_code", 200)
        ).alignment = Alignment(horizontal="center")
        ...
        ws_perf.cell(
            row=row_idx, column=4, value=p.get("render_time_ms") or "-"
        ).alignment = Alignment(horizontal="center")
        ws_perf.cell(row=row_idx, column=5, value=m_score).alignment = Alignment(
            horizontal="center"
        )
        ws_perf.cell(row=row_idx, column=6, value=d_score).alignment = Alignment(
            horizontal="center"
        )
