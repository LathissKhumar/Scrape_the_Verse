...
export type HealingPhase =
  "idle" | "running" | "failure" | "healing" | "recovered";
...
export function useSelfHealingSequence() {
  ...
  const start = useCallback(() => {
    ...
    timeouts.current = [
      ...
      schedule(() => {
        setPhase("failure");
        addEvent("warning", "⚠ Website structure changed");
      }, 2000),
      schedule(() => addEvent("error", "⚠ Selector no longer found"), 2400),
      schedule(() => addEvent("error", "⚠ Extraction returned empty"), 2700),
      schedule(() => {
        setPhase("healing");
        addEvent("healing", "SELF-HEALING AGENT ACTIVATED");
      }, 3000),
      schedule(
        () => addEvent("healing", "Analyzing new page structure…"),
        3800,
      ),
      ...
      schedule(() => {
        setPhase("recovered");
        addEvent("success", "✓ Collector restored — 129 records extracted");
      }, 6800),
    ];
  }, [addEvent]);
