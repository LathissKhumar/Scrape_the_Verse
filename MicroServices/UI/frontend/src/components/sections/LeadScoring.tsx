...
const SCORES = [
  ...
  { label: 'Opportunity Value', value: 90, color: 'from-emerald-400 to-cyan-400' },
  { label: 'Evidence Confidence', value: 85, color: 'from-violet-400 to-indigo-400' },
      ...
      <div className="max-w-4xl mx-auto px-6 lg:px-8 space-y-14">
        ...
        >
          ...
          <p className="text-base text-text-secondary font-body max-w-2xl mx-auto leading-relaxed">
            Every lead is automatically scored across five dimensions so your team works the
            highest-value opportunities first.
          </p>
        </motion.div>

        {/* Score bars */}
        <div ref={ref} className="glass-level-2 p-8 md:p-10 rounded-3xl space-y-8">
          {SCORES.map((score, i) => (
            <div key={score.label} className="space-y-2.5">
              <div className="flex items-center justify-between text-sm font-mono">
                <span className="text-slate-200 font-semibold">{score.label}</span>
              ...
              <div className="h-2.5 rounded-full bg-white/8 overflow-hidden">
                <motion.div
                  ...
                  initial={{ width: '0%' }}
                  animate={isInView ? { width: `${score.value}%` } : { width: '0%' }}
                  transition={{
                    duration: 1.0,
                    delay: i * 0.15,
          ...
          <div className="pt-4 border-t border-white/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <p className="text-xs font-body text-slate-500 max-w-sm leading-relaxed">
              Scores are evidence-based — derived from what the agents actually found, not assumptions.
            </p>
          </div>
        </div>
      </div>
