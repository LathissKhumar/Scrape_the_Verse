...
export function SalesAutomation() {
  ...
  const activeAgent =
    AI_AGENTS.find((a) => a.id === activeAgentId) ?? AI_AGENTS[0];
  ...
  return (
    <section
      ...
      aria-label="Autonomous AI Sales Suite"
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl font-bold font-display tracking-tight text-text-primary">
            ...
            <GradientText>automated sales action.</GradientText>
          </h2>
          <p className="text-base text-text-secondary max-w-xl mx-auto font-body">
            Scrape-Verse doesn&apos;t just collect data. Dedicated AI agents
            generate tailored landing pages, personalized emails, and voice
            scripts.
          </p>
        </motion.div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`p-3 rounded-xl ${isActive ? "bg-violet-accent/20" : "bg-white/5"}`}
                    >
                  ...
                  <ArrowRight
                    className={`w-4 h-4 transition-transform ${isActive ? "text-violet-accent translate-x-1" : "text-muted"}`}
                  />
                </motion.div>
              );
            })}
            ...
            <div className="glass-level-3 p-8 space-y-6 border-white/20 shadow-2xl relative overflow-hidden">
              ...
              <div className="space-y-4">
                ...
                <div className="p-5 rounded-2xl bg-black/40 border border-white/10 font-mono text-xs text-text-secondary leading-relaxed space-y-2">
                  <div className="text-violet-accent font-semibold">
                    {"// Target: Urban Brew Café"}
                  </div>
                  <div className="whitespace-pre-line">
                    {activeAgent.outputSnippet}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between border-t border-white/10 pt-4 font-mono text-xs text-muted">
                <span>
                  Agent Status:{" "}
                  <strong className="text-emerald-success">Active</strong>
                </span>
                <Button
                  id="deploy-agent-btn"
                  variant="primary"
                  className="!text-xs !px-5 !py-2 flex items-center gap-1.5 shadow-lg shadow-violet-accent/20"
                >
