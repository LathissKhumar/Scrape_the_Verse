...
export const triggerDisintegration = (
  ...
  if (typeof window !== "undefined") {
    window.dispatchEvent(
      new CustomEvent("trigger-disintegration", { detail: { x: cx, y: cy } }),
    );
  }
};
...
interface ParticleDisintegrationTransitionProps {
  ...
  triggerRef?: React.MutableRefObject<
    ((clickEvent?: React.MouseEvent | MouseEvent) => void) | null
  >;
}

export const ParticleDisintegrationTransition: React.FC<
  ParticleDisintegrationTransitionProps
> = ({ targetHref = "/dashboard", triggerRef }) => {
  ...
  const samplePageElements = (clickX: number, clickY: number): Particle[] => {
    ...
    const bottomDock = {
      x: vw / 2,
      y: vh - 35,
      width: Math.min(vw * 0.65, 680),
      height: 48,
    };

    const getReconstructTarget = (
      i: number,
      ...
      if (rand < 20) {
        ...
        return {
          x: topBarTargets.x + spread,
          y: topBarTargets.y + (Math.random() - 0.5) * topBarTargets.height,
          z: 0,
        };
      } else if (rand < 50) {
        ...
        return {
          x: bottomDock.x + spread,
          y: bottomDock.y + (Math.random() - 0.5) * bottomDock.height,
          z: 0,
        };
      }
    };
    ...
    elements.forEach((el) => {
      ...
      if (el.id === "landing-glass-frame" || el.id === "landing-svg-mask")
        return;
      ...
      const isButton =
        ...
        el.id?.includes("btn");
      const isCard =
        el.className?.toString().includes("glass") ||
      ...
      const elDelay = (elDist / Math.max(vw, vh)) * 0.4;
      ...
      if (
        isButton ||
        el.className?.toString().includes("sky") ||
        style.color.includes("56") ||
        style.borderColor.includes("56")
      ) {
        r = 56;
        g = 189;
        b = 248;
      } else if (
        style.color.includes("96") ||
        style.background.includes("60A5FA")
      ) {
        r = 96;
        g = 165;
        b = 250;
      ...
      for (let py = rect.top; py <= rect.bottom; py += step) {
        for (let px = rect.left; px <= rect.right; px += step) {
          ...
          const angle =
            Math.atan2(jy - clickY, jx - clickX) + (Math.random() - 0.5) * 0.85;
  ...
  const startDisintegration = useCallback(
    (clickX?: number, clickY?: number) => {
      ...
      const render = (now: number) => {
        ...
        const p3 = Math.max(0, Math.min(1, (progress - 0.6) / 0.3));
        const p4 = Math.max(0, Math.min(1, (progress - 0.9) / 0.1));
        ...
        for (let d = 0; d < dissolvingList.length; d++) {
          ...
          if (!item.dissolved && progress >= item.delay) {
            ...
            item.el.style.transition =
              "opacity 0.40s cubic-bezier(0.4, 0, 0.2, 1), transform 0.50s cubic-bezier(0.4, 0, 0.2, 1), filter 0.35s ease-out";
            item.el.style.filter =
              "blur(6px) brightness(1.8) drop-shadow(0 0 12px rgba(56, 189, 248, 0.9))";
        ...
        if (progress > 0.3 && progress <= 0.75) {
          cameraZ = Math.sin(((progress - 0.3) / 0.45) * Math.PI) * 200;
        }
        ...
        for (let i = 0; i < particles.length; i++) {
          ...
          if (p.active) {
            if (progress < 0.6) {
              ...
              p.x = p.x + (p.targetX - p.x) * (0.1 + easeT * 0.2);
              p.y = p.y + (p.targetY - p.y) * (0.1 + easeT * 0.2);
              p.z = p.z + (p.targetZ - p.z) * (0.1 + easeT * 0.2);
