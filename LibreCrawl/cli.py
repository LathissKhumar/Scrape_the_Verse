...
def save_csv_output(result: Dict[str, Any], filepath: str):
    ...
    fieldnames = [
        ...
        'word_count', 'canonical', 'robots', 'response_time_ms'
    ]
...
def main(args_list: Optional[list] = None) -> int:
    ...
    parser.add_argument("--max-pages", "-m", type=int, default=100, help="Maximum pages to crawl (default: 100)")
    parser.add_argument("--delay", type=float, default=0.05, help="Delay between requests in seconds (default: 0.05)")
    parser.add_argument("--concurrency", "-c", type=int, default=5, help="Number of concurrent crawl workers (default: 5)")
    
    # Flags with positive/negative option pairs
    parser.add_argument("--javascript", "--js", action=argparse.BooleanOptionalAction, default=False, help="Enable/disable JavaScript rendering via Playwright")
    parser.add_argument("--pagespeed", "--ps", action=argparse.BooleanOptionalAction, default=False, help="Enable/disable Google PageSpeed analysis")
    parser.add_argument("--respect-robots", action=argparse.BooleanOptionalAction, default=True, help="Respect/ignore robots.txt rules")
    parser.add_argument("--discover-sitemaps", action=argparse.BooleanOptionalAction, default=True, help="Discover and parse sitemaps")
    parser.add_argument("--external-links", action=argparse.BooleanOptionalAction, default=False, help="Include external links in link analysis")
    parser.add_argument("--images", action=argparse.BooleanOptionalAction, default=False, help="Crawl and check image URLs")

    parser.add_argument("--output", "-o", type=str, help="Path to output file (JSON or CSV)")
    parser.add_argument("--format", "-f", choices=["json", "csv"], default="json", help="Output format (default: json)")
    parser.add_argument("--json", action="store_true", help="Output machine JSON directly to stdout")
    parser.add_argument("--quiet", "-q", action="store_true", help="Suppress non-essential terminal output")
    parser.add_argument("--serve", action="store_true", help="Start the headless JSON REST API server")
    ...
    if not args.url:
        if args.json or not sys.stdin.isatty():
            err_json = format_error_result("INVALID_URL", "Target URL is required. Provide --url <URL>.")
    ...
    if not is_valid:
        if json_stdout:
            print(json.dumps(format_error_result("INVALID_URL", err_msg, {"url": target_url}), indent=2))
        else:
    ...
    if result.get("status") == "failed":
        if json_stdout:
            ...
            print(f"\nCrawl failed [{err.get('code', 'UNKNOWN')}]: {err.get('message', '')}", file=sys.stderr)
        return 1
    ...
    if output_file:
        try:
            ...
            if json_stdout:
                print(json.dumps(format_error_result("FILE_WRITE_ERROR", f"Could not write to output file: {str(e)}"), indent=2))
            else:
                print(f"Warning: Could not save output to {output_file}: {e}", file=sys.stderr)
            return 3
