    ...
    >
      ...
      <nav className="relative max-w-7xl mx-auto px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <a href="#" className="flex items-center gap-3 group" id="nav-logo">
          <div
            className="w-9 h-9 rounded-xl flex items-center justify-center text-xs font-mono font-bold shadow-lg shadow-violet-accent/20 transition-transform group-hover:scale-105"
            style={{
        ...
        <ul className="hidden md:flex items-center gap-8" role="list">
          {NAV_LINKS.map((link) => (
            <li key={link.href}>
              <a
                href={link.href}
                className="text-sm font-medium font-body text-text-secondary hover:text-text-primary transition-colors duration-200"
              >
                {link.label}
              </a>
            </li>
        ...
        <div className="flex items-center gap-4">
          ...
          <Button id="nav-cta" variant="primary" className="!text-xs !px-5 !py-2.5">
            Get Started
          </Button>
        </div>
      </nav>
