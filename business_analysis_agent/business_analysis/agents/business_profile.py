...
def business_profile_agent(state: BusinessAnalysisState) -> BusinessAnalysisState:
    ...
    try:
        ...
        if profile.industry.status == FieldStatusEnum.UNKNOWN and input_biz.industry:
            profile.industry = FieldStatus(
                value=input_biz.industry,
                status=FieldStatusEnum.KNOWN,
                confidence=1.0,
                evidence_ids=[
                    relevant_evidence[0].id if relevant_evidence else "fact_001"
                ],
            )
        if (
            profile.geographic_market.status == FieldStatusEnum.UNKNOWN
            and input_biz.location
        ):
            profile.geographic_market = FieldStatus(
                value=input_biz.location,
                status=FieldStatusEnum.KNOWN,
                confidence=1.0,
                evidence_ids=[
                    relevant_evidence[0].id if relevant_evidence else "fact_001"
        ...
        return {
            **state,
            "errors": state.get("errors", []) + [f"BusinessProfileAgent error: {e!s}"],
