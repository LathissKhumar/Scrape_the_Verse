...
def market_analysis_agent(state: BusinessAnalysisState) -> BusinessAnalysisState:
    ...
    services_val = ", ".join([s.name for s in service_analysis.services]) if service_analysis and service_analysis.services else "Dental Services"
    ...
    try:
        ...
        statuses["market_analysis"] = NodeExecutionStatus(status=NodeStatusEnum.SUCCESS, confidence=0.88)
        return {**state, "market_analysis": analysis, "node_statuses": statuses}
    except Exception as e:
        statuses["market_analysis"] = NodeExecutionStatus(status=NodeStatusEnum.FAILED, confidence=0.0, error_message=str(e))
        return {**state, "errors": state.get("errors", []) + [f"MarketAnalysisAgent error: {str(e)}"], "node_statuses": statuses}