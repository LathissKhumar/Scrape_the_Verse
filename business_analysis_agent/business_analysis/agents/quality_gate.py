...
def quality_gate_agent(state: BusinessAnalysisState) -> BusinessAnalysisState:
    ...
    if profile and getattr(profile.industry, "value", "Not specified") not in (
        "Not specified",
        "",
        None,
    ):
        passed_checks.append(
            "Business Profile: industry and location explicitly grounded in evidence."
        )
    else:
        failed_checks.append(
            "CRITICAL: Business Profile missing explicit industry or location — analysis cannot be trusted."
        )

    # Check 2: Services extracted from evidence
    if service_analysis and service_analysis.services:
        passed_checks.append(
            f"Service extraction: {len(service_analysis.services)} services found from evidence."
        )
    else:
        failed_checks.append(
            "CRITICAL: No services extracted despite available evidence."
        )
    ...
    if problems:
        ...
        if problems_with_evidence:
            passed_checks.append(
                f"Evidence grounding: {len(problems_with_evidence)}/{len(problems)} problems have evidence IDs."
            )
        else:
            failed_checks.append(
                "Business problems exist but NONE have evidence_ids — evidence chain broken."
            )
    else:
    ...
    if opportunities:
        ...
        if opps_with_refs:
            passed_checks.append(
                f"Opportunity→Problem chain: {len(opps_with_refs)}/{len(opportunities)} opportunities reference a problem."
            )
        else:
            failed_checks.append(
                "Opportunities present but none reference a business problem — chain broken."
            )
    else:
    ...
    if opportunities and any(o.recommended_services for o in opportunities):
        passed_checks.append(
            "Agency service taxonomy: at least one opportunity maps to agency services."
        )
    else:
    ...
    if score and score.analysis_completeness > 0:
        passed_checks.append(
            f"Scoring: completeness is {score.analysis_completeness}% (non-zero)."
        )
    else:
        failed_checks.append(
            "Business score completeness is 0 — analysis may be empty."
        )
    ...
    for node_name in _CRITICAL_NODES:
        ...
        if st == NodeStatusEnum.FAILED:
            ...
            failed_checks.append(
                f"CRITICAL: Node '{node_name}' was SKIPPED — analysis incomplete."
            )
        else:
    ...
    for node_name, label in [
        ...
        if st == NodeStatusEnum.SKIPPED:
            warnings.append(
                f"{label} was SKIPPED — market/customer/competitor findings unavailable."
            )
    ...
    if completeness:
        ...
        skipped_market = statuses.get(
            "market_analysis", NodeExecutionStatus()
        ).status in [NodeStatusEnum.SKIPPED, NodeStatusEnum.FAILED]
        if skipped_market and completeness.market_completeness > 0:
            warnings.append(
                f"Market completeness reported as {completeness.market_completeness}% but node was SKIPPED/FAILED — inconsistency detected."
            )

    # Check 10: Website analysis availability
    if website_analysis is not None:
        passed_checks.append(
            f"Website analysis available (status: {getattr(website_analysis, 'crawl_status', 'UNKNOWN')})."
        )
    else:
        warnings.append(
        ...
        notes.append(
            "Website analysis can be supplied externally via WebsiteAnalysisResult schema."
        )

    # Check 11: Customer segments
    if customer_analysis and customer_analysis.segments:
        passed_checks.append(
            f"Customer segments: {len(customer_analysis.segments)} segments derived."
        )
    else:
        warnings.append(
            "No customer segments derived — customer analysis is incomplete."
        )

    # Check 12: Competitor research vs not performed
    if competitor_analysis and competitor_analysis.competitors:
        passed_checks.append(
            f"Competitor analysis: {len(competitor_analysis.competitors)} competitors identified."
        )
    elif competitor_analysis and not competitor_analysis.competitors:
        warnings.append(
            "Competitor analysis node ran but found no competitors — candidate discovery may have failed."
        )
    else:
    ...
    passed_checks.append(
        "FACT/INFERENCE/RECOMMENDATION separation: evidence-backed extraction applied in all agents."
    )
    ...
    if critical_failures or failed_checks:
    ...
    updated_statuses["quality_gate"] = NodeExecutionStatus(
        status=NodeStatusEnum.SUCCESS, confidence=1.0
    )

    # Accumulate warnings into state
    existing_warnings = [
        e for e in state.get("errors", []) if e.startswith("[WARNING]")
    ]
