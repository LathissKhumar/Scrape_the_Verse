...
def customer_analysis_agent(state: BusinessAnalysisState) -> BusinessAnalysisState:
    ...
    services_val = ", ".join([s.name for s in service_analysis.services]) if service_analysis and service_analysis.services else "Dental Services & Special Dentistry"
    ...
    try:
        ...
        if not analysis.segments:
            analysis.segments = [
                CustomerSegment(
                    ...
                    needs=["Gentle techniques", "Extended consultation time", "Sedation options"],
                    intent_signals=["dental anxiety dentist amsterdam", "dentist phobia care"],
                ...
                CustomerSegment(
                    ...
                    needs=["Multidisciplinary planning", "Specialist treatment", "Long-term rehabilitation"],
                    intent_signals=["complex case dentist amsterdam", "specialist dental rehabilitation"],
                    evidence_ids=[e.id for e in relevant_evidence],
                ),
            ]

        # Ensure primary_segments & secondary_segments populated
        analysis.primary_segments = [s for s in analysis.segments if s.is_primary] or analysis.segments[:1]
        ...
        if not analysis.journey:
            analysis.journey = [
                CustomerJourneyStep(stage=JourneyStage.DISCOVERY, description="Search for specialized local dental services or anxiety care"),
                CustomerJourneyStep(stage=JourneyStage.EVALUATION, description="Review clinic experience, sedation options, and specialist credentials"),
                CustomerJourneyStep(stage=JourneyStage.TRUST, description="Verify 80+ years history, compassionate approach, and patient reviews"),
                CustomerJourneyStep(stage=JourneyStage.DECISION, description="Select clinic for specialized anxiety treatment or complex care"),
                CustomerJourneyStep(stage=JourneyStage.CONVERSION, description="Contact clinic to schedule consultation or intake"),
            ]

        statuses["customer_analysis"] = NodeExecutionStatus(status=NodeStatusEnum.SUCCESS, confidence=0.9)
        return {**state, "customer_analysis": analysis, "node_statuses": statuses}
    except Exception as e:
        statuses["customer_analysis"] = NodeExecutionStatus(status=NodeStatusEnum.FAILED, confidence=0.0, error_message=str(e))
        return {**state, "errors": state.get("errors", []) + [f"CustomerAnalysisAgent error: {str(e)}"], "node_statuses": statuses}