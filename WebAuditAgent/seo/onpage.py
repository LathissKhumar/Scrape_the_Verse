...
def run_onpage_audit(pages: List[Dict[str, Any]], issues: List[Dict[str, Any]]) -> CategoryAuditResult:
    ...
    short_titles = [p for p in indexable_pages if (p.get('title') or '').strip() and len(p.get('title', '').strip()) < 30]
    long_titles = [p for p in indexable_pages if len((p.get('title') or '').strip()) > 60]
    ...
    missing_desc = [p for p in indexable_pages if not (p.get('meta_description') or '').strip()]
    if missing_desc:
    ...
    for p in indexable_pages:
        ...
        unlabeled = [img for img in imgs if isinstance(img, dict) and not (img.get('alt') or '').strip()]
        if unlabeled:
